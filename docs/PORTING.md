...
| AWTK | No GPU → can't use T507's `egl_devices/mc` (EGL/Mali). Runs as an mc client via a software LCD device `awtk-port/lcd_devices/mc/lcd_mc.c` (`LCD_DEVICES=mc_sw`): `lcd_mem` BGRA8888 pointed at the compositor's dma-buf (`mc_surface_buf_at`), committed per frame (`mc_surface_commit_idx` + buffer rotate). NANOVG-AGGE software canvas. Input via `input_thread_mc` (no tslib). Selected by the `t113_mc_awtk_config_define.py` config; built+packaged by `build.sh T113` (guarded). Note: `mc` (not `mc_sw`) is already the T507 EGL-backed mc device, hence the distinct name. G2D-accelerated AWTK rendering is a possible later optimization. |
