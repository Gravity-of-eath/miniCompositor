...
log "sourcing $PLATFORM toolchain env"
# shellcheck disable=SC1090
source "$T5SDK_ENV"

log "building mc compositor + tools (Makefile)"
run make CROSS=1 MC_ENABLE_EGL=1 -j"$JOBS"

log "building LVGL demos"
run make CROSS=1 demos -j"$JOBS"
...
INPUT="${MC_INPUT:-/dev/input/event1}"  # touchscreen (st1633i on carbit T507)

# 1. compositor (Mali GPU compose on /dev/fb0)
