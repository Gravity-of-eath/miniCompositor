...
ret_t image_base_on_event(widget_t* widget, event_t* e) {
  ...
  switch (type) {
    case EVT_TOUCH_DOWN: {
      printf("image_base_on_event line:%d EVT_TOUCH_DOWN image->clickable=%d\n",__LINE__,image->clickable);
      image->pressed = TRUE;
      if (image->clickable) {
    ...
    case EVT_POINTER_DOWN:
      ...
      break;
    case EVT_TOUCH_UP: {
      printf("image_base_on_event line:%d EVT_TOUCH_UP image->clickable=%d image->pressed=%d\n",__LINE__,image->clickable,image->pressed);
      touch_event_t* up = touch_event_cast(e);
      if (image->pressed) {
        if ((image->clickable) &&
