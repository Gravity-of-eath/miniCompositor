...
#ifdef WITH_LCD_LINUX_MC
  /* mc routes touch from the compositor (already hit-tested + translated
   * to surface-local coords). Skip all /dev/input/* threads to avoid
   * double-reading the same physical device. */
  (void)info;
  (void)loop;
  return RET_OK;
#else
...
#ifdef WITH_LCD_LINUX_MC
  {
    tk_thread_t *mc_in = input_thread_mc_run(input_dispatch_to_main_loop,
                                             loop, loop->w, loop->h);
    if (mc_in != NULL) {
      slist_append(&s_device_threads_list, mc_in);
    } else {
      log_warn("WITH_LCD_LINUX_MC: input_thread_mc_run failed; no touch input\n");
    }
  }
#endif
