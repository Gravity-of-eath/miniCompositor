...
def lcd_devices_is_egl(lcd_devices):
  if lcd_devices == 'egl_for_fsl' or lcd_devices == 'egl_for_t507' or lcd_devices == 'egl_for_x11' or lcd_devices == 'egl_for_gbm' or lcd_devices == 'egl_for_wayland':
    return True
  # 'mc' is also an EGL-backed lcd device: it uses lcd_linux_egl.c +
  # the mc backend in awtk-port/egl_devices/mc/ which renders into
  # ion dma-bufs imported as FBO color attachments. From the AWTK
  # build's perspective it looks the same as any other EGL device.
  if lcd_devices == 'mc':
    return True
  return False
...
COMMON_CCFLAGS=COMMON_CCFLAGS+' -DWITH_SOCKET=1 '
...
elif LCD_DEVICES =='egl_for_t507':
  #for egl for fsl
  OS_FLAGS=OS_FLAGS + ' -DEGL_API_FB '
  OS_LIBS = [ 'GLESv2', 'EGL' ] + OS_LIBS
elif LCD_DEVICES =='mc':
  # zero-copy AWTK -> mc-compositor: GPU writes into ion dma-bufs the
  # compositor owns; AWTK app code is unchanged, lcd_linux_egl.c is
  # reused; egl_devices/mc/egl_devices.c is the EGL backend.
