...
ret_t qr_set_value(widget_t* widget, const char* value) {
  ...
  if (qr->last_value != NULL && strcmp(qr->last_value, value) == 0) {
    return RET_OK;
  }
  ...
  if (qr->qrcode != NULL) {
    ...
    qr->qrcode = NULL;
    canvas_offline_destroy(qr->cache);
    qr->cache = NULL;
  }

  qr->last_value = tk_str_copy(qr->last_value, value);
...
static ret_t qr_on_paint_self(widget_t* widget, canvas_t* c) {
  ...
  if (style != NULL) {
    ...
    if (qr_ensure_qrcode(widget) == RET_OK) {
      ...
      if (qr->cache == NULL) {
        qr->cache = canvas_offline_create(widget->w, widget->h, BITMAP_FMT_RGBA8888);
        canvas_offline_clear_canvas(qr->cache);
        canvas_offline_begin_draw(qr->cache);
        ...
        for (j = 0; j < size; j++) {
          ...
          for (i = 0; i < size; i++) {
            if (*p & 0x01) {
              ...
              canvas_set_fill_color(qr->cache, fg);
            } else {
              canvas_set_fill_color(c, bg);
              canvas_set_fill_color(qr->cache, bg);
            }
            canvas_fill_rect(c, x, y, pix_size, pix_size);
            canvas_fill_rect(qr->cache, x, y, pix_size, pix_size);
            p++;
            x += pix_size;
          }
          y += pix_size;
        }
        canvas_offline_end_draw(qr->cache);
      } else {
        rect_t* r = rect_create(0, 0, widget->w, widget->h);
        rect_t* r1 = rect_create(0, 0, widget->w, widget->h);
        canvas_draw_image(c, canvas_offline_get_bitmap(qr->cache), r, r1);
        rect_destroy(r);
        rect_destroy(r1);
      }
...
ret_t qr_on_attach_parent(widget_t* widget, widget_t* parent) {
  printf("qr_on_attach_parent\n");
  return RET_OK;
}
...
TK_DECL_VTABLE(qr) = {.size = sizeof(qr_t),
                      ...
                      .create = qr_create,
                      .on_attach_parent = qr_on_attach_parent,
