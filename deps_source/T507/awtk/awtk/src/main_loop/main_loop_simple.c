  ...
  while ((time_out - time_in < 20) && (main_loop_recv_event((main_loop_t*)loop, &r) == RET_OK)) {
    ...
    switch (r.event.type) {
      ...
      case EVT_TOUCH_UP: {
        ...
        touch_event_t* e = (touch_event_t*)(&r.touch_event);
        widget_dispatch(win, e);
        if (r.event.type == EVT_TOUCH_UP) {
          touch_event_t touch_event;
          touch_event_init(&touch_event, EVT_TOUCH_OVER, e->e.target, 0, -1, e->x, e->y, 0);
          widget_dispatch_recursive(win, (event_t*)&touch_event);
        }
        break;
      }
