...
static ret_t button_on_event(widget_t* widget, event_t* e) {
  ...
  switch (type) {
    ...
    case EVT_TOUCH_UP: {
      pointer_event_t touch;
      touch_event_t* up = (touch_event_t*)(e);
      ENSURE(up);
      pointer_event_init(&touch, EVT_CLICK, widget, up->x, up->y);

      if (button->pressed && widget_is_point_in(widget, touch.x, touch.y, FALSE)) {
        button_pointer_up_cleanup(widget);
        ret = widget_dispatch(widget, (event_t*)(&touch));
      } else {
        button_pointer_up_cleanup(widget);
        widget_set_state(widget, WIDGET_STATE_NORMAL);
      }
