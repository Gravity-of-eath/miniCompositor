...
void* egl_devices_create(const char* filename) {
  printf("egl_devices_create fbdev: %s\n", filename);
  egl_devices_fb_context_t* ctx = TKMEM_ZALLOC(egl_devices_fb_context_t);
  ...
  fb_info(filename, &width, &height);
  ctx->eglNativeWindow = (EGLNativeWindowType)createNativeWindow(width, height);		
...
ret_t egl_devices_dispose(void* ctx) {
  egl_devices_fb_context_t* context = (egl_devices_fb_context_t*)ctx;
